import copy
import math
from mnist.slang_mnist_continue2 import *
import mnist.slang_mnist_continue as slang_mnist_continue

experiment_name = "slang_mnist_continue3"
...
mnist_continue3 = slang_mnist_continue.slang_continue.add_variant(experiment_name,
                             data_set=data_set,
...
for L in Ls:
    ...
    # Load state dicts
    try:
        exp = slang_mnist_continue.slang_continue.get_variant("slang_mnist_continue2").get_variant(best_variant)
        ...
        params = copy.deepcopy(mnist_continue3.get_args())
        params['optimizer_params']['decay_params'] = { 'learning_rate': decay, 'beta': decay }
        params['optimizer_params']['L'] = L
        name = 'L_{}'.format(L)
        variants.append(name)
        mnist_continue3.add_variant(name, optimizer_params=params['optimizer_params'], model_state_dict=model_state_dict, optimizer_state_dict=optimizer_state_dict)