import copy
import math
from mnist.slang_mnist_val import *
import mnist.slang_mnist_continue as slang_mnist_continue
import slang_experiments as slang

experiment_name = "slang_mnist_continue1"
...
mnist_continue = slang_mnist_continue.slang_continue.add_variant(experiment_name,
                             data_set=data_set,
...
for L in Ls:
    ...
    best_variant = 'L_{}_pp_{:.2f}_dec_{:.2f}'.format(L, prior_prec, decay)

    # Load state dicts
    exp = slang.slang_base.get_variant("slang_mnist_val").get_variant(best_variant)
    try:
        ...
        params = copy.deepcopy(mnist_continue.get_args())
        params['optimizer_params']['decay_params'] = { 'learning_rate': decay, 'beta': decay }
        params['optimizer_params']['L'] = L
        name = 'L_{}'.format(L)
        variants.append(name)
        mnist_continue.add_variant(name, optimizer_params=params['optimizer_params'], model_state_dict=model_state_dict, optimizer_state_dict=optimizer_state_dict)