...
class Unf_MeanFieldLayer(nn.Module):
    ...
    def __init__(self, input_dim, output_dim, init_var=5e-2, prior_var = 1):
        super(Unf_MeanFieldLayer, self).__init__()
    ...
    def q_w(self):
        ...
        return torch.distributions.Uniform(self.w_mu_q - (0.5 * self.w_log_var_q).exp(), self.w_mu_q + (0.5 * self.w_log_var_q).exp())
...
class Unf_MeanFieldBNN(nn.Module):
    ...
    def __init__(
        ...
        super(Unf_MeanFieldBNN, self).__init__()
        self.input_dim = input_dim
        ...
        for i in range(len(hidden_dims) + 1):
            if i == 0:
                self.network.append(Unf_MeanFieldLayer(self.input_dim, self.hidden_dims[i], prior_var = prior_var))
                self.network.append(self.activation)
            elif i == len(hidden_dims):
                self.network.append(
                    Unf_MeanFieldLayer(self.hidden_dims[i - 1], self.output_dim, prior_var = prior_var)
                )
            else:
                self.network.append(
                    Unf_MeanFieldLayer(self.hidden_dims[i - 1], self.hidden_dims[i], prior_var = prior_var)
                )
                self.network.append(self.activation) 
    ...
    def kl(self):
        ...
        return sum([layer.kl() for layer in self.network if isinstance(layer, Unf_MeanFieldLayer)])
